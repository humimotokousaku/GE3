...
void SceneTransition::Initialize(bool isBackGround) {
	...
	indexResource_->SetName(L"IndexBuffer");
...
void SceneTransition::Draw(int textureNum) {
	// このスプライトが背景なら
	if (isBackGround_) {
	...
	DirectXCommon::GetInstance()->GetCommandList()->SetGraphicsRootSignature(MyEngine::GetInstance()->GetRootSignature().Get());
	DirectXCommon::GetInstance()->GetCommandList()->SetPipelineState(MyEngine::GetInstance()->GetGraphicsPipelineState().Get()); // PSOを設定
