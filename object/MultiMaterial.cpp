...
Microsoft::WRL::ComPtr<ID3D12Resource> MultiMaterial::CreateBufferResource(const Microsoft::WRL::ComPtr<ID3D12Device>& device, size_t sizeInBytes) {
	HRESULT hr;
	// 頂点リソース用のヒープの設定
...
void MultiMaterial::CreateMaterialResource() {
...
void MultiMaterial::CreateWvpResource() {
	// 1つ分のサイズを用意する
...
void MultiMaterial::Initialize() {
	// モデルを読み込み
	modelData_ = ObjManager::GetInstance()->GetObjModelData()[MULTIMATERIAL];
...
void MultiMaterial::Draw() {
	if (isAlive_) {
		...
		// DescriptorTableの設定
		DirectXCommon::GetInstance()->GetCommandList()->SetGraphicsRootDescriptorTable(2, TextureManager::GetInstance()->GetTextureSrvHandleGPU()[5]);
...
void MultiMaterial::Release() {

}

void MultiMaterial::ImGuiAdjustParameter() {
